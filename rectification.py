...
def compute_votes(edgelets, model, threshold_inlier=5):
    ...
    # Preprocess input to arccos to avoid floating point errors causing errors
    temp = np.abs(cosine_theta)
    for idx, val in enumerate(temp):
        if val >= 1.0:
            val = 0.9999999999999999999999999999
        temp[idx] = val
...
def ransac_3_line(edgelets, focal_length=910, num_ransac_iter=5000,
                  threshold_inlier=5):
    ...
    for ransac_iter in range(num_ransac_iter):
        ...
        h = [vp1[0]*(1 / focal_length**2), vp1[1]*(1 / focal_length**2), vp1[2]]
        vp2 = np.cross(h, l3)
...
def calc_homography_only(image, vp1, vp2, clip=True, clip_factor=3):
    # Find Projective Transform
    vanishing_line = np.cross(vp1, vp2)
    H = np.eye(3)
    H[2] = vanishing_line / vanishing_line[2]
    ...
    return H #np.dot(T, inter_matrix)


def get_homography(image, clip_factor=6, algorithm='independent', 
                  reestimate=False):
    ...
    if algorithm == 'independent':
        ...
        vp1 = ransac_vanishing_point(edgelets1, 10000, threshold_inlier=5)
        if reestimate:
        ...
        vp2 = ransac_vanishing_point(edgelets2, 10000, threshold_inlier=5)
        if reestimate:
            vp2 = reestimate_model(vp2, edgelets2, 5)
    elif algorithm == '3-line':
        vp1, vp2 = ransac_3_line(edgelets1, num_ransac_iter=3000, threshold_inlier=5)
    else:
        raise KeyError(
            "Parameter 'algorithm' has to be one of {'3-line', 'independent'}")

    # Compute the homography and warp
    return calc_homography_only(image, vp1, vp2,
